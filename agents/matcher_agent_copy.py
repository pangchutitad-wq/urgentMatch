...
def generate_doctors_on_duty():
    """Generate random number of doctors on duty (2-4)."""
...
async def handle_match_request(ctx: Context, sender: str, msg: MatchRequest):
    try:
        ...
        # Convert to ClinicResult format for frontend
        results = []
        for score, clinic in ranked:
            ...
            results.append(
                ClinicResult(
                    ...
                    doctorsOnDuty=generate_doctors_on_duty(),
                    rating=clinic.rating,
                    reviewCount=0,  # Would come from Google Places in production
